import os, random, functools, logging, json, glob
import numpy as np
...
def load_cityscapes_samples(data_dir, split, cfg):
    ...
    logger.info("Preprocessing Cityscapes object detection annotations of {} set...".format(split))
...
def cityscapes_sample_to_dict(file, is_train, domain, label_space):
    ...
    for obj in jsonobj["objects"][::-1]:
        ...
        if label_space.__name__ != "Cityscapes":
            err_msg = "You are trying to map classes of {} label space into Cityscapes format.".format(str(label_space.__name__))
            raise ValueError(err_msg)
        else:
            try:
        ...
        contiguous_id_dict={l.id: idx for idx, l in enumerate(label_space.instance_labels)}
...
def register_foggy_cityscapes(cfg, root_dir="datasets"):
    ...
    label_space = get_label_space(cfg.FOOC.DATASETS.FOGGYCITYSCAPES.LABEL_SPACE)
...
def load_foggy_cityscapes_samples(data_dir, split, cfg):
    ...
    logger.info("Preprocessing FoggyCityscapes object detection annotations of {} set...".format(split))
    ...
    if cfg.FOOC.DATASETS.FOGGYCITYSCAPES.SUBSAMPLE == 0:
        ...
        files = random.sample(files, cfg.FOOC.DATASETS.FOGGYCITYSCAPES.SUBSAMPLE)
    assert len(files)
...
def load_sim10k_samples(data_dir, split, cfg):
    ...
    logger.info("Preprocessing Sim10k object detection annotations of {} set...".format(split))
    ...
    if cfg.FOOC.DATASETS.SIM10K.SUBSAMPLE == 0:
        ...
        image_ids = random.sample(image_ids, cfg.FOOC.DATASETS.SIM10K.SUBSAMPLE)
...
def sim10k_sample_to_dict(image_id, data_dir, is_train, domain, label_space):
    ...
    image_path=os.path.join(data_dir, "VOC2012", "JPEGImages", f"{image_id}.jpg")
    annos_path=os.path.join(data_dir, "VOC2012", "Annotations", f"{image_id}.xml")
    ...
    for obj in tree.findall("object"):
        ...
        if label_space.__name__ not in ["Sim10k", "Sim10kCityscapesCommon"]:
            err_msg = "You are trying to map classes of {} label space into Sim10k format.".format(str(label_space.__name__))
            raise ValueError(err_msg)
        else:
...
def load_kitti_samples(data_dir, split, cfg):
    ...
    logger.info("Preprocessing kitti object detection annotations of {} set...".format(split))
...
def kitti_sample_to_dict(image_id, data_dir, is_train, domain, label_space):
    ...
    """
    # determine the storage path of the image and its label based on its ID
    image_path=os.path.join(data_dir, "training", "image_2", f"{image_id}.png")
    annos_path=os.path.join(data_dir, "training", "label_2", f"{image_id}.txt")
    assert os.path.exists(image_path)
    assert os.path.exists(annos_path)
    ...
    for instance in instances:
        ...
        if label_space.__name__ not in ["Kitti", "KittiCityscapesCommon"]:
            err_msg = "You are trying to map classes of {} label space into Kitti format.".format(str(label_space.__name__))
            raise ValueError(err_msg)
        else:
