...
def get_detection_dataset_dicts(names, filter_empty=True, min_keypoints=0, proposal_files=None, equal_frequency=True):
    """
...
def _train_loader_from_config(cfg, mapper=None, *, dataset=None, sampler=None):
    if dataset is None:
        dataset, dataset_name_to_indices = get_detection_dataset_dicts(
            cfg.DATASETS.TRAIN,
...
# ==================== Datasets Merging ==================== #
def match_dataset_dicts_lengths(dataset_dicts_list):
    """
    Makes sure all datasets are of the same length by repeating the shorter ones until they match
    the longest one.
    Args:
        dataset_dicts_list list[list[dict]]: The registered datasets. First level of nested lists
            corresponds to the different datasets, while the second level corresponds to the
            annotation examples in a datset.
    Returns:
        dataset_dicts_list list[list[dict]]: The registered datasets but with matched lengths.
    """
    dataset_lengths = list(map(len, dataset_dicts_list))

    if not len(set(dataset_lengths)) > 1:
        logger = logging.getLogger(__name__)
        if len(dataset_lengths) == 1:
            logger.warning(
                "Sampling frequency matching is enabled but only a single dataset was supplied."
            )
        else:
            logger.warning(
                "All datasets are already the same length. No frequency matching is applied."
            )
        return dataset_dicts_list

    highest_dataset_length = max(dataset_lengths)
    dataset_dicts_list = [
        [dataset_dict for _, dataset_dict in zip(range(highest_dataset_length), dataset_dicts)]
...
def merge_dataset_dicts_list(dataset_dicts_list, equal_frequency: bool):
    """
    Concatenates the elements of the datasets dicts list. In case equal_frequency is True,
    shorter dataset are repeated till the length of the longest dataset is met.
    Args:
        dataset_dicts_list (list[list[dict]]): list of dataset dicts for all datasets
        equal_frequency (bool): flag indicating whether the datasets should be brought to same length
            before concatenation
    Returns:
        the concatenated dataset dicts (list[dict]),
        list of indices corresponding to the newly created dataset dict list, one for each dataset
                in `dataset_dicts_list` (list[list[int]])
    """
    if equal_frequency:
        dataset_dicts_list = match_dataset_dicts_lengths(dataset_dicts_list)

    dataset_indices = []
    start_index = 0
    for dataset_dicts in dataset_dicts_list:
